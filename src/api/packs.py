...
@router.post("/packs/catalog/", response_model=List[Pack])
def get_catalog(wholesale_catalog: List[Pack]):
    """Returns a list of all booster packs available for purchase"""
    print(f"barrel catalog: {wholesale_catalog}")

@router.post("/users/{user_id}/inventory", response_model=List[Pack])
def get_inventory(user_id: int):
    ...
    with db.engine.begin() as connection:
        owned_packs = connection.execute(
            sqlalchemy.text( # p.cost may be renamed to p.price?
                """
                SELECT p.name, p.cost, i.quantity FROM inventory as i
    ...
    for pack_data in owned_packs:
        pack_inventory.append(Pack(name=pack_data[0], price=pack_data[1], quantity=pack_data[2]))
    
    return pack_inventory

@router.post("/users/{user_id}/purchase_packs", status_code=status.HTTP_204_NO_CONTENT)
